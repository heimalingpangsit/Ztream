...
object NetworkModule {
    ...
    // ---------- Service Nobar realtime (chat room + undangan) ----------
    // Dipisah dari authClient karena butuh pingInterval buat jaga WebSocket
    // tetap hidup, dan read timeout yang lebih longgar buat polling.

    @Provides
    @Singleton
    @Named("nobarClient")
    fun provideNobarOkHttpClient(): OkHttpClient {
        return OkHttpClient.Builder()
    ...
    @Provides
    @Singleton
    fun provideNobarRealtimeApi(
        @Named("nobarClient") okHttpClient: OkHttpClient,
        json: Json,
    ): com.zaaam.zreming.data.remote.NobarRealtimeApi {
        val contentType = "application/json".toMediaType()
        return Retrofit.Builder()
            .baseUrl(com.zaaam.zreming.data.remote.NobarRealtimeConfig.BASE_HTTP_URL)
            .client(okHttpClient)
            .addConverterFactory(json.asConverterFactory(contentType))
            .build()
            .create(com.zaaam.zreming.data.remote.NobarRealtimeApi::class.java)
    }
}
