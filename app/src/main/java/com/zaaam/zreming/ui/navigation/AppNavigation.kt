            ...
            if (showBottomBar) {
                Surface(
                    ...
                    NavigationBar(
                        ...
                        NavigationBarItem(
                            selected = currentRoute == Screen.NobarHub.route,
                            onClick = {
                                navController.navigate(Screen.NobarHub.route) {
                                    popUpTo(navController.graph.findStartDestination().id) {
                                        saveState = true
                                    }
                                    launchSingleTop = true
                            ...
                            icon = { Icon(Icons.Default.Groups, contentDescription = "Join Nobar") },
                            label = { Text("Join Nobar", fontSize = 11.sp) },
                            colors = NavigationBarItemDefaults.colors(
                                selectedIconColor = PrimaryRed,
                                selectedTextColor = PrimaryRed,
                                indicatorColor = Color.White.copy(alpha = 0.1f),
        ...
        NavHost(
            ...
            composable(Screen.Login.route) {
                LoginScreen(
                    ...
                    onOpenBeliAccount = { navController.navigate(Screen.BeliAccount.route) },
                    onOpenPulihkanAccount = { navController.navigate(Screen.PulihkanAccount.route) },
                )
            }

            composable(Screen.BeliAccount.route) {
                com.zaaam.zreming.ui.webpage.AssetWebPageScreen(
                    assetFileName = "order.html",
                    title = "Beli Account",
                    onBack = { navController.popBackStack() },
                )
            }

            composable(Screen.PulihkanAccount.route) {
                com.zaaam.zreming.ui.webpage.AssetWebPageScreen(
                    assetFileName = "helper.html",
                    title = "Pulihkan Account",
                    onBack = { navController.popBackStack() },
                )
            }
            ...
            composable(Screen.Profile.route) {
                ProfileScreen(
                    ...
                    onOpenAboutUs = { navController.navigate(Screen.AboutUs.route) },
                    onLoggedOut = {
            ...
            composable(route = Screen.NobarHub.route) {
                com.zaaam.zreming.ui.nobar.NobarHubScreen(
                    onOpenRoom = { roomId ->
                        navController.navigate(
                            // title/contentId/dll di-default kosong di sini karena kita cuma
                            // punya roomId — PlayerViewModel akan ambil detail konten yang
                            // sebenarnya dari GET /api/nobar/{roomId} begitu Player dibuka
                            // (lihat catatan di PlayerViewModel soal ini).
                            Screen.Player.createRoute("Nobar", "0", false, 1, 1, 0L, "", roomId)
                        ) {
                            popUpTo(Screen.NobarHub.route) { inclusive = false }
                        }
                    }
                )
            }
